        ...
        (async () => {
            try {
                ...
                allSyms.forEach((sym) => {
                    ...
                    displayRounds.forEach((r) => {
                        // qty held *after* the previous round
                        const prevQty =
                            prevValue === null
                                ? 0
                                : data[r].find((h) => h.symbol === sym)?.quantity ?? 0;

                        // Actually, for the very first round we want 0×price
                        const qty =
                            prevValue === null
                                ? 0
                                : data[r].find((h) => h.symbol === sym)?.quantity ?? 0;
                        ...
                        const price = data[r].find((h) => h.symbol === sym)?.currentPrice ?? 0;
                        ...
                        prevValue = value; // bump for next iteration
                    });
                });
