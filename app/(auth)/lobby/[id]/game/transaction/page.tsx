...
const TransactionPage: React.FC = () => {
    ...
    const handleSubmitRound = () => {
         ...
         // ... (gather buy/sell amounts with current prices) ...

         console.log("Submitting Transactions:", transactions);
         if (transactions.length > 0) {
              // --- API call to submit transactions ---
              message.success("Round submitted! Redirecting...");
              // ... handle API response ...
              setTimeout(() => router.push(`/lobby/${gameId}/leader_board`), 1000);
         } else {
              message.info("No transactions to submit. Redirecting...");
              setTimeout(() => router.push(`/lobby/${gameId}/leader_board`), 1000);
         }
    };
