// app/game/transaction/symbol/[symbol]/page.tsx
"use client";
        ...
        const fetchChartData = async () => {
            ...
            try {
                ...
                if (historyData && historyData.length > 0) {
                    ...
                    const hasRequiredData = historyData.every(d =>
                        d.date != null && d.open != null && d.high != null && d.low != null && d.close != null
                    );
                    if (hasRequiredData) {
                         setChartData(historyData);
                    } else {
                         console.warn(`Data for ${symbol} is missing required fields (open, high, low, close) for some points.`);
                         setError(`Chart data for ${symbol} is incomplete.`);
                    }
                } else {
                    setError(`No historical data found for ${symbol}. Data might not be fetched yet.`);
                    message.info(`No historical data found for ${symbol}. Try fetching data via the admin endpoint if available.`);
                }
            } catch (err: any) {
                console.error(`Failed to fetch chart data for ${symbol}`, err);
                setError(`Could not load chart data for ${symbol}. ${err.message || 'Please try again later.'}`);
